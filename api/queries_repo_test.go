...
func TestRepoMetadata(t *testing.T) {
	...
	repo, _ := ghrepo.FromFullName(literal_9408)
	...
	if !sliceEqual(memberIDs, expectedMemberIDs) {
		t.Errorf(literal_6045, expectedMemberIDs, memberIDs)
	}
...
func TestProjectsToPaths(t *testing.T) {
...
func TestProjectNamesToPaths(t *testing.T) {
	http := &httpmock.Registry{}
	client := newTestClient(http)
...
func TestRepoResolveMetadataIDs(t *testing.T) {
	http := &httpmock.Registry{}
	client := newTestClient(http)
...
`
	...
	if !sliceEqual(teamIDs, expectedTeamIDs) {
		t.Errorf(literal_6045, expectedTeamIDs, teamIDs)
	}
	...
	if !sliceEqual(labelIDs, expectedLabelIDs) {
		t.Errorf(literal_6045, expectedLabelIDs, labelIDs)
	}
}
...
func TestRepoMilestones(t *testing.T) {
	tests := []struct {
		state   string
		want    string
		wantErr bool
	...
	tests := []struct {
	...
	}{
		{
			...
			httpStub: func(r *httpmock.Registry) {
				r.Register(
					httpmock.REST("HEAD", literal_6752),
					httpmock.StringResponse("{}"),
				)
			},
...
const literal_3976 = "unexpected error: %v"

const literal_6045 = "expected members %v, got %v"

const literal_6752 = "repos/OWNER/REPO"
