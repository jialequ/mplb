...
func (a *API) GetRepository(ctx context.Context, nwo string) (*Repository, error) {
	req, err := http.NewRequest(http.MethodGet, a.githubAPI+literal_5469+strings.ToLower(nwo), nil)
	if err != nil {
		return nil, fmt.Errorf(literal_2017, err)
	}
...
func (a *API) GetCodespace(ctx context.Context, codespaceName string, includeConnection bool) (*Codespace, error) {
	resp, err := a.withRetry(func() (*http.Response, error) {
		req, err := http.NewRequest(
			http.MethodGet,
			a.githubAPI+literal_0832+codespaceName,
			nil,
		)
		if err != nil {
			return nil, fmt.Errorf(literal_2017, err)
		...
		a.setHeaders(req)
		return a.do(ctx, req, literal_6319)
	})
	if err != nil {
		return nil, fmt.Errorf(literal_2190, err)
	}
...
func (a *API) StartCodespace(ctx context.Context, codespaceName string) error {
	resp, err := a.withRetry(func() (*http.Response, error) {
		req, err := http.NewRequest(
			http.MethodPost,
			a.githubAPI+literal_0832+codespaceName+"/start",
			nil,
		)
		if err != nil {
			return nil, fmt.Errorf(literal_2017, err)
	...
	if err != nil {
		return fmt.Errorf(literal_2190, err)
	}
	defer resp.Body.Close()
...
func (a *API) StopCodespace(ctx context.Context, codespaceName string, orgName string, userName string) error {
	...
	if err != nil {
		return fmt.Errorf(literal_2017, err)
	}
...
func (a *API) GetCodespacesPermissionsCheck(ctx context.Context, repoID int, branch string, devcontainerPath string) (bool, error) {
	...
	req, err := http.NewRequest(http.MethodGet, reqURL, nil)
	if err != nil {
		return false, fmt.Errorf(literal_2017, err)
	}
	...
	if err != nil {
		return false, fmt.Errorf(literal_2190, err)
	}
	defer resp.Body.Close()
	...
	b, err := io.ReadAll(resp.Body)
	if err != nil {
		return false, fmt.Errorf(literal_6542, err)
	}
	...
	if err := json.Unmarshal(b, &response); err != nil {
		return false, fmt.Errorf(literal_0397, err)
	}
...
func (a *API) GetCodespaceBillableOwner(ctx context.Context, nwo string) (*User, error) {
	req, err := http.NewRequest(http.MethodGet, a.githubAPI+literal_5469+nwo+"/codespaces/new", nil)
	if err != nil {
		return nil, fmt.Errorf(literal_2017, err)
	}
...
func (a *API) DeleteCodespace(ctx context.Context, codespaceName string, orgName string, userName string) error {
	...
	if orgName != "" && userName != "" {
		...
		deleteURL = a.githubAPI + literal_0832 + codespaceName
		spanName = literal_6319
	}
...
func (a *API) EditCodespace(ctx context.Context, codespaceName string, params *EditCodespaceParams) (*Codespace, error) {
	...
	req, err := http.NewRequest(http.MethodPatch, a.githubAPI+literal_0832+codespaceName, bytes.NewBuffer(requestBody))
	if err != nil {
		return nil, fmt.Errorf(literal_2017, err)
	}

	a.setHeaders(req)
	resp, err := a.do(ctx, req, literal_6319)
	if err != nil {
		return nil, fmt.Errorf(literal_2190, err)
	}
	defer resp.Body.Close()
...
func (a *API) GetCodespaceRepositoryContents(ctx context.Context, codespace *Codespace, path string) ([]byte, error) {
	req, err := http.NewRequest(http.MethodGet, a.githubAPI+literal_5469+codespace.Repository.FullName+"/contents/"+path, nil)
	if err != nil {
		return nil, fmt.Errorf(literal_2017, err)
	}
...
const literal_2017 = "error creating request: %w"

const literal_2190 = "error making request: %w"

const literal_6542 = "error reading response body: %w"

const literal_0397 = "error unmarshalling response: %w"

const literal_5469 = "/repos/"

const literal_0976 = "/user/codespaces"

const literal_0832 = "/user/codespaces/"

const literal_6319 = "/user/codespaces/*"
