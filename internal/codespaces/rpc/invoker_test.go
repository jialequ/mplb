...
import (
	...
	"github.com/jialequ/mplb/internal/codespaces/rpc/codespace"
	"github.com/jialequ/mplb/internal/codespaces/rpc/jupyter"
	"github.com/jialequ/mplb/internal/codespaces/rpc/ssh"
	rpctest "github.com/jialequ/mplb/internal/codespaces/rpc/test"
...
func TestStartJupyterServerFailure(t *testing.T) {
	...
	if err.Error() != errorMessage {
		t.Fatalf(literal_7315, errorMessage, err)
	}
	if port != 0 {
		t.Fatalf("expected %d, got %d", 0, port)
	}
	if url != "" {
		t.Fatalf(literal_6754, "", url)
	}
...
func TestStartSSHServerSuccess(t *testing.T) {
	...
	if user != resp.User {
		t.Fatalf(literal_6754, resp.User, user)
	}
...
func TestStartSSHServerFailure(t *testing.T) {
	...
	if user != "" {
		t.Fatalf(literal_6754, "", user)
	}
}

const literal_6719 = "error connecting to internal server: %v"

const literal_7315 = "expected %v, got %v"

const literal_6754 = "expected %s, got %s"
