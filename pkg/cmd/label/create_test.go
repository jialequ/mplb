...
func TestNewCmdCreate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			output: createOptions{Name: "test", Description: literal_2319},
		},
		{
			name:   "color flag",
...
func TestCreateRun(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "creates label",
			tty:  true,
			opts: &createOptions{Name: "test", Description: literal_2319},
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(
					httpmock.REST("POST", literal_9875),
		...
		},
		{
			...
			opts: &createOptions{Name: "test", Description: literal_2319, Force: true},
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(
					httpmock.REST("POST", literal_9875),
					httpmock.WithHeader(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, literal_2319, reqBody["description"])
			assert.Equal(t, "test", reqBody["name"])
		})
	}
}

const literal_2319 = "some description"

const literal_9875 = "repos/OWNER/REPO/labels"
