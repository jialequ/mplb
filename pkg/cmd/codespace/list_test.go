...
func TestAppList(t *testing.T) { //NOSONAR
	type fields struct {
		apiClient apiClient
	}
	tests := []struct {
			...
			fields: fields{
				apiClient: &apiClientMock{
					ListCodespacesFunc: func(ctx context.Context, opts api.ListCodespacesOptions) ([]*api.Codespace, error) {
						...
						if opts.UserName != "" {
							return nil, fmt.Errorf(literal_1375, opts.UserName)
						}
						return []*api.Codespace{
							{
								DisplayName: "CS1",
...
const literal_3718 = "2023-01-01T00:00:00Z"

const literal_1375 = "Expected userName to be blank. Got %s"

const literal_9285 = "cli/cli"
