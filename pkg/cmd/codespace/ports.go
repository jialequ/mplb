...
import (
	...
	"github.com/jialequ/mplb/internal/codespaces"
	"github.com/jialequ/mplb/internal/codespaces/api"
	"github.com/jialequ/mplb/internal/codespaces/portforwarder"
...
func (a *App) ListPorts(ctx context.Context, selector *CodespaceSelector, exporter cmdutil.Exporter) (err error) {
	...
	if err != nil {
		return fmt.Errorf(literal_0984, err)
	}

	fwd, err := portforwarder.NewPortForwarder(ctx, codespaceConnection)
	if err != nil {
		return fmt.Errorf(literal_2840, err)
	}
...
const literal_0984 = "error connecting to codespace: %w"

const literal_2840 = "failed to create port forwarder: %w"
