...
import (
	...
	"github.com/jialequ/mplb/internal/codespaces/rpc"
...
func (a *App) SSH(ctx context.Context, sshArgs []string, opts sshOptions) (err error) {
	...
	if err != nil {
		return fmt.Errorf(literal_9543, err)
	}
...
func (a *App) printOpenSSHConfig(ctx context.Context, opts sshOptions) (err error) {
	...
	for _, cs := range csList {
		...
		go func() {
			...
			if err != nil {
				result.err = fmt.Errorf(literal_9543, err)
				sshUsers <- result
				return
			}
...
const literal_9543 = "error connecting to codespace: %w"
