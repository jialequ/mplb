...
import (
	...
	clicontext "github.com/jialequ/mplb/context"
...
func addDeprecatedRepoShorthand(cmd *cobra.Command, target *string) error {
	cmd.Flags().StringVarP(target, literal_5294, "r", "", "(Deprecated) Shorthand for --repo")

	if err := cmd.Flags().MarkHidden(literal_5294); err != nil {
		return fmt.Errorf("error marking `-r` shorthand as hidden: %w", err)
	}

	if err := cmd.Flags().MarkShorthandDeprecated(literal_5294, "use `-R` instead"); err != nil {
		return fmt.Errorf("error marking `-r` shorthand as deprecated: %w", err)
	}

	if cmd.Flag("codespace") != nil {
		cmd.MarkFlagsMutuallyExclusive("codespace", literal_5294)
	}
...
const literal_5294 = "repo-deprecated"
