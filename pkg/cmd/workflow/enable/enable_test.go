	...
	tests := []struct {
	...
	}{
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(
					httpmock.REST("GET", literal_4390),
				...
				reg.Register(
					httpmock.REST("PUT", literal_0358),
					httpmock.StatusStringResponse(204, "{}"))
			},
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect("Select a workflow", []string{literal_4052}, func(_, _ string, opts []string) (int, error) {
					return prompter.IndexFor(opts, literal_4052)
				})
			},
			wantOut: literal_7649,
		},
		{
			name: "tty name arg",
			opts: &EnableOptions{
		...
		},
		{
			...
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect("Which workflow do you mean?", []string{literal_4052, "a disabled workflow (anotherDisabled.yml)"}, func(_, _ string, opts []string) (int, error) {
...
const literal_4390 = "repos/OWNER/REPO/actions/workflows"

const literal_0358 = "repos/OWNER/REPO/actions/workflows/456/enable"

const literal_4052 = "a disabled workflow (disabled.yml)"

const literal_7649 = "✓ Enabled a disabled workflow\n"
