...
import (
	...
	"github.com/jialequ/mplb/pkg/cmd/workflow/shared"
...
jobs:
  ...
  - name: Send greeting
	...
	stubs := func(reg *httpmock.Registry) {
		reg.Register(
			httpmock.REST("GET", literal_2457),
			httpmock.JSONResponse(shared.Workflow{
				Path: literal_2803,
				ID:   12345,
			}))
		reg.Register(
			httpmock.REST("POST", literal_2309),
	...
	tests := []struct {
		...
		},
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(
					httpmock.REST("GET", literal_2541),
		...
		},
		{
			...
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect(literal_2768, []string{"minimal workflow (minimal.yml)"}, func(_, _ string, opts []string) (int, error) {
					return 0, nil
				})
			},
			wantBody: map[string]interface{}{
		...
		},
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				...
				reg.Register(
					httpmock.REST("GET", literal_4372),
					httpmock.JSONResponse(struct{ Content string }{
			...
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect(literal_2768, []string{"a workflow (workflow.yml)"}, func(_, _ string, opts []string) (int, error) {
					return 0, nil
				})
		...
		},
		{
			...
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect(literal_2768, []string{"choice inputs (workflow.yml)"}, func(_, _ string, opts []string) (int, error) {
					return 0, nil
				})
		...
		},
		{
			...
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect(literal_2768, []string{"choice missing inputs (workflow.yml)"}, func(_, _ string, opts []string) (int, error) {
					return 0, nil
				})
...
const literal_2457 = "repos/OWNER/REPO/actions/workflows/workflow.yml"

const literal_2803 = ".github/workflows/workflow.yml"

const literal_2309 = "repos/OWNER/REPO/actions/workflows/12345/dispatches"

const literal_2794 = "✓ Created workflow_dispatch event for workflow.yml at trunk\n\nTo see runs for this workflow, try: gh run list --workflow=workflow.yml\n"

const literal_2541 = "repos/OWNER/REPO/actions/workflows"

const literal_2768 = "Select a workflow"

const literal_4372 = "repos/OWNER/REPO/contents/.github/workflows/workflow.yml"
