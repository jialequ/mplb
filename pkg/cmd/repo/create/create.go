...
import (
	...
	"github.com/jialequ/mplb/pkg/cmd/repo/shared"
...
func NewCmdCreate(f *cmdutil.Factory, runF func(*CreateOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed(literal_3948) {
				opts.DisableIssues = !enableIssues
			}
			if cmd.Flags().Changed(literal_2134) {
	...
	cmd.Flags().BoolVar(&enableIssues, literal_3948, true, "Enable issues in the new repository")
	cmd.Flags().BoolVar(&enableWiki, literal_2134, true, "Enable wiki in the new repository")

	_ = cmd.Flags().MarkDeprecated("confirm", "Pass any argument to skip confirmation prompt")
	_ = cmd.Flags().MarkDeprecated(literal_3948, "Disable issues with `--disable-issues`")
	_ = cmd.Flags().MarkDeprecated(literal_2134, "Disable wiki with `--disable-wiki`")
...
func createFromScratch(opts *CreateOptions) error {
	...
	if strings.Contains(opts.Name, "/") {
		...
		if err != nil {
			return fmt.Errorf(literal_2149, err)
		}
	} else {
...
const literal_3948 = "enable-issues"

const literal_2134 = "enable-wiki"

const literal_2149 = "argument error: %w"

const literal_5682 = "%s Created repository %s on GitHub\n  %s\n"
