...
func TestRepoFork(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			tty:  true,
			opts: &ForkOptions{
				Repository: literal_5902,
			},
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(
					httpmock.REST("POST", literal_5603),
					httpmock.StringResponse(forkResult))
			},
			wantErr: true,
		...
		},
		{
			...
			httpStubs: forkPost,
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterConfirm(literal_2798, func(_ string, _ bool) (bool, error) {
					return false, nil
				})
			},
			wantErrOut: "✓ Created fork someone/REPO\n",
...
const literal_2634 = "git remote rename origin upstream"

const literal_5902 = "someone/REPO"

const literal_5603 = "repos/someone/REPO/forks"

const literal_4068 = "someone/REPO already exists\n"

const literal_0256 = "OWNER/REPO"

const literal_9058 = "✓ Created fork someone/REPO\n✓ Cloned fork\n! Repository OWNER/REPO set as the default repository. To learn more about the default repository, run: gh repo set-default --help\n"

const literal_2798 = "Would you like to clone the fork?"
