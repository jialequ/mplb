	...
	tests := []struct {
	...
	}{
		{
			...
			mockAPI: func(p *mockPlatform) {
				p.On("List", "2345").Return([]shared.Artifact{
					...
					{
						Name:        literal_7832,
						DownloadURL: literal_7190,
						Expired:     false,
					},
				}, nil)
				p.On("Download", literal_2079, filepath.FromSlash("tmp/artifact-1")).Return(nil)
				p.On("Download", literal_7190, filepath.FromSlash("tmp/artifact-2")).Return(nil)
			},
		},
		{
		...
		},
		{
			...
			mockAPI: func(p *mockPlatform) {
				...
				p.On("Download", literal_7190, ".").Return(nil)
			},
			promptStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterMultiSelect("Select artifacts to download:", nil, []string{literal_4569, literal_7832},
...
const literal_4569 = "artifact-1"

const literal_2079 = "http://download.com/artifact1.zip"

const literal_7832 = "artifact-2"

const literal_7190 = "http://download.com/artifact2.zip"
