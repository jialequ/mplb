...
func NewCmdDevelop(f *cmdutil.Factory, runF func(*DevelopOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			flags := cmd.Flags()
			if flags.Changed(literal_4705) {
				if flags.Changed("repo") {
					if flags.Changed(literal_5824) {
						return cmdutil.FlagErrorf("specify only `--repo` and `--branch-repo`")
					}
					branchRepo, _ := flags.GetString("repo")
					_ = flags.Set(literal_5824, branchRepo)
				}
				repo, _ := flags.GetString(literal_4705)
	...
	fl := cmd.Flags()
	fl.StringVar(&opts.BranchRepo, literal_5824, "", "Name or URL of the repository where you want to create your new branch")
	...
	fl.StringVarP(&issueRepoSelector, literal_4705, "i", "", "Name or URL of the issue's repository")
	_ = cmd.Flags().MarkDeprecated(literal_4705, "use `--repo` instead")
...
const literal_4705 = "issue-repo"

const literal_5824 = "branch-repo"
