	...
	tests := []struct {
		...
		},
		{
			...
			httpStubs: func(reg *httpmock.Registry, t *testing.T) {
				...
				reg.Register(
					httpmock.GraphQL(`mutation CreateLinkedBranch\b`),
					httpmock.GraphQLMutation(`{"data":{"createLinkedBranch":{"linkedBranch":{"id":"2","ref":{"name":literal_3764}}}}}`,
						func(inputs map[string]interface{}) {
							assert.Equal(t, "REPOID", inputs["repositoryId"])
							assert.Equal(t, "SOMEID", inputs["issueId"])
							assert.Equal(t, "OID", inputs["oid"])
							assert.Equal(t, literal_3764, inputs["name"])
						}),
				)
			},
			runStubs: func(cs *run.CommandStubber) {
...
const literal_6142 = "OWNER/REPO"

const literal_3764 = "my-branch"

const literal_9706 = "github.com/OWNER/REPO/tree/my-branch\n"
