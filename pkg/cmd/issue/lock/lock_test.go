...
func TestNewCmdLock(t *testing.T) {
	cases := []struct {
		name    string
		args    string
		want    LockOptions
...
func TestNewCmdUnlock(t *testing.T) {
	cases := []struct {
		name    string
		args    string
		want    LockOptions
		...
		},
		{
			...
			promptStubs: func(t *testing.T, pm *prompter.PrompterMock) {
				pm.SelectFunc = func(p, d string, opts []string) (int, error) {
					if p == "Lock reason?" {
						assert.Equal(t, []string{"None", "Off topic", "Resolved", "Spam", literal_6145}, opts)

						return prompter.IndexFor(opts, literal_6145)
					}
