...
func TestPRStatus_currentBranch_showTheMostRecentPR(t *testing.T) {
	...
	if !expectedLine.MatchString(output.String()) {
		t.Errorf(literal_0639, expectedLine, output)
		return
	}
...
`
	if output.String() != expected {
		t.Errorf(literal_7945, expected, output.String())
	}
}
...
func TestPrSelectorForCurrentBranch(t *testing.T) {
	rs, cleanup := run.Stub()
	defer cleanup(t)
...
const literal_2638 = "error running command `pr status`: %v"

const literal_0639 = "output did not match regexp /%s/\n> output\n%s\n"

const literal_7945 = "expected %q, got %q"
