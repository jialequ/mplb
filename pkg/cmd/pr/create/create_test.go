				...
				reg.Register(
					...
					httpmock.GraphQLMutation(`
						...
						func(input map[string]interface{}) {
							...
							assert.Equal(t, literal_6524, input["title"])
							assert.Equal(t, literal_9352, input["body"])
			...
			httpStubs: func(reg *httpmock.Registry, t *testing.T) {
				...
				reg.Register(
					...
					httpmock.GraphQLMutation(`
						...
						assert.Equal(t, literal_6524, input["title"].(string))
						assert.Equal(t, literal_9352, input["body"].(string))
...
func TestDetermineTrackingBranch(t *testing.T) {
	tests := []struct {
		name     string
		cmdStubs func(*run.CommandStubber)
		...
		},
		{
			...
			remotes: context.Remotes{
				&context.Remote{
					...
					Repo:   ghrepo.New("hubot", literal_4136),
				},
				&context.Remote{
					Remote: &git.Remote{Name: "upstream"},
					Repo:   ghrepo.New("octocat", literal_4136),
				},
			},
			assert: func(ref *git.TrackingRef, t *testing.T) {
				assert.Nil(t, ref)
...
func TestGenerateCompareURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "basic",
			ctx: CreateContext{
				BaseRepo:        api.InitRepoHostname(&api.Repository{Name: "REPO", Owner: api.RepositoryOwner{Login: "OWNER"}}, literal_7029),
				BaseBranch:      "main",
...
const literal_9352 = "my body"

const literal_5403 = "https://github.com/OWNER/REPO/pull/12\n"

const literal_2869 = "/robots"

const literal_2309 = "big one.oh"

const literal_6318 = "Where should we push the 'feature' branch?"

const literal_5860 = "\nCreating pull request for feature into master in OWNER/REPO\n\n"

const literal_3807 = "What's next?"

const literal_1736 = "git -c log.ShowSignature=false log --pretty=format:%H%x00%s%x00%b%x00 --cherry origin/master...feature"

const literal_4136 = "Spoon-Knife"

const literal_7029 = "github.com"
