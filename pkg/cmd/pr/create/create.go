...
import (
	...
	ghContext "github.com/jialequ/mplb/context"
...
func NewCmdCreate(f *cmdutil.Factory, runF func(*CreateOptions) error) *cobra.Command { //NOSONAR
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			noMaintainerEdit, _ := cmd.Flags().GetBool(literal_6478)
			...
			if cmd.Flags().Changed(literal_6478) && opts.WebMode {
	...
	fl.Bool(literal_6478, false, "Disable maintainer's ability to modify pull request")
...
func renderPullRequestTTY(io *iostreams.IOStreams, params map[string]interface{}, state *shared.IssueMetadataState) error {
	...
	fmt.Fprintf(out, literal_6784, iofmt.Bold("Title"), params["title"].(string))
	fmt.Fprintf(out, "%s: %t\n", iofmt.Bold("Draft"), params["draft"])
	fmt.Fprintf(out, literal_6784, iofmt.Bold("Base"), params["baseRefName"])
	fmt.Fprintf(out, literal_6784, iofmt.Bold("Head"), params["headRefName"])
	if len(state.Labels) != 0 {
		fmt.Fprintf(out, literal_6784, iofmt.Bold("Labels"), strings.Join(state.Labels, ", "))
	}
	if len(state.Reviewers) != 0 {
		fmt.Fprintf(out, literal_6784, iofmt.Bold("Reviewers"), strings.Join(state.Reviewers, ", "))
	}
	if len(state.Assignees) != 0 {
		fmt.Fprintf(out, literal_6784, iofmt.Bold("Assignees"), strings.Join(state.Assignees, ", "))
	}
	if len(state.Milestones) != 0 {
		fmt.Fprintf(out, literal_6784, iofmt.Bold("Milestones"), strings.Join(state.Milestones, ", "))
	}
	if len(state.Projects) != 0 {
		fmt.Fprintf(out, literal_6784, iofmt.Bold("Projects"), strings.Join(state.Projects, ", "))
	}
...
const literal_6478 = "no-maintainer-edit"

const literal_6784 = "%s: %s\n"
