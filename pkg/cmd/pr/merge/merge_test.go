...
func TestPrMerge(t *testing.T) {
	...
	http.Register(
		...
		httpmock.GraphQLMutation(`{}`, func(input map[string]interface{}) {
			assert.Equal(t, literal_5410, input["pullRequestId"].(string))
	...
	output, err := runCommand(http, nil, "main", true, literal_1639)
	if err != nil {
		t.Fatalf(literal_4183, err)
	}

	r := regexp.MustCompile(`Merged pull request OWNER/REPO#1 \(The title of the PR\)`)

	if !r.MatchString(output.Stderr()) {
		t.Fatalf(literal_9514, r, output.Stderr())
	}
}
...
func TestPrMerge_nontty(t *testing.T) {
	...
	output, err := runCommand(http, nil, "main", false, literal_1639)
	if err != nil {
		t.Fatalf(literal_4183, err)
	}
...
func TestPrMerge_deleteBranch(t *testing.T) {
	...
	http.Register(
		httpmock.REST("DELETE", literal_2603),
...
func TestNonDivergingPullRequest(t *testing.T) {
	http := initFakeHTTP()
	defer http.Verify(t)
	...
	output, err := runCommand(http, nil, "blueberries", true, literal_6975)
	if err != nil {
		t.Fatalf(literal_4183, err)
	}
...
func TestDivergingPullRequestWarning(t *testing.T) {
	http := initFakeHTTP()
	defer http.Verify(t)
...
func TestPullRequestWithoutCommits(t *testing.T) {
	http := initFakeHTTP()
	defer http.Verify(t)
...
func TestPrMerge_alreadyMerged_withMergeStrategy(t *testing.T) {
	...
	output, err := runCommand(http, nil, "blueberries", false, literal_4192)
	if err != nil {
		t.Fatalf(literal_4821, err)
	}
...
func TestPrMerge_alreadyMerged_withMergeStrategy_TTY(t *testing.T) {
	...
	output, err := runCommand(http, pm, "blueberries", true, literal_4192)
	if err != nil {
		t.Fatalf(literal_4821, err)
	}
...
func TestPRMergeTTY_squashEditCommitMsgAndSubject(t *testing.T) {
	...
	err := mergeRun(&MergeOptions{
		...
		Prompter:           pm,
		SelectorArg:        literal_1405,
		MergeStrategyEmpty: true,
		Finder: shared.NewMockFinder(
			literal_1405,
			&api.PullRequest{ID: literal_5410, Number: 123, Title: "title", MergeStateStatus: "CLEAN"},
...
func TestPRMergeEmptyStrategyNonTTY(t *testing.T) {
	...
	output, err := runCommand(http, nil, "blueberries", false, literal_1097)
...
func TestMergeMethodSurvey(t *testing.T) {
	repo := &api.Repository{
...
func TestMergeRun_autoMerge(t *testing.T) {
	...
	err := mergeRun(&MergeOptions{
		...
		Finder: shared.NewMockFinder(
			literal_1405,
			&api.PullRequest{ID: literal_5410, Number: 123, MergeStateStatus: "BLOCKED"},
			ghrepo.New("OWNER", "REPO"),
		),
	})
	assert.NoError(t, err)
...
func TestMergeRun_autoMerge_directMerge(t *testing.T) {
	...
	err := mergeRun(&MergeOptions{
		...
		Finder: shared.NewMockFinder(
			literal_1405,
			&api.PullRequest{ID: literal_5410, Number: 123, MergeStateStatus: "CLEAN"},
			ghrepo.New("OWNER", "REPO"),
		),
	})
	assert.NoError(t, err)
...
func TestMergeRun_disableAutoMerge(t *testing.T) {
	...
	tr.Register(
		...
		httpmock.GraphQLQuery(`{}`, func(s string, m map[string]interface{}) {
			assert.Equal(t, map[string]interface{}{"prID": literal_5410}, m)
		}))
	...
	err := mergeRun(&MergeOptions{
		...
		Finder: shared.NewMockFinder(
			literal_1405,
			&api.PullRequest{ID: literal_5410, Number: 123},
			ghrepo.New("OWNER", "REPO"),
		),
	})
	assert.NoError(t, err)
...
func TestPrInMergeQueue(t *testing.T) {
	...
	output, err := runCommand(http, nil, "blueberries", true, literal_1097)
	if err != nil {
		t.Fatalf(literal_4183, err)
	}
...
func TestPrAddToMergeQueueWithMergeMethod(t *testing.T) {
	...
	output, err := runCommand(http, nil, "blueberries", true, literal_1639)
	if err != nil {
		t.Fatalf(literal_4183, err)
	}
	assert.Equal(t, "", output.String())
...
const literal_5410 = "THE-ID"

const literal_2749 = "The title of the PR"

const literal_1639 = "pr merge 1 --merge"

const literal_4183 = "error running command `pr merge`: %v"

const literal_9514 = "output did not match regexp /%s/\n> output\n%q\n"

const literal_3209 = "Blueberries are a good fruit"

const literal_2603 = "repos/OWNER/REPO/git/refs/heads/blueberries"

const literal_4821 = "Got unexpected error running `pr merge` %s"

const literal_6975 = "pr merge --merge"

const literal_0423 = "Pull request OWNER/REPO#4 was already merged. Delete the branch locally?"

const literal_4192 = "pr merge 4 --merge"

const literal_6987 = "Delete the branch locally and on GitHub?"

const literal_8106 = "What's next?"

const literal_3589 = "What merge method would you like to use?"

const literal_1405 = "https://github.com/OWNER/REPO/pull/123"

const literal_1097 = "pr merge 1"
