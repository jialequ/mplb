...
func TestNewCmdSet(t *testing.T) {
	tests := []struct {
		...
		{
			name: "repo",
			cli:  `cool_variable -bliteral_9270`,
			wants: SetOptions{
				VariableName: "cool_variable",
		...
		{
			name: "env",
			cli:  `cool_variable -bliteral_9270 -eRelease`,
			wants: SetOptions{
				VariableName: "cool_variable",
				Visibility:   shared.Private,
				Body:         literal_9270,
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			opts := &SetOptions{
				...
				BaseRepo: func() (ghrepo.Interface, error) {
					return ghrepo.FromFullName(literal_9281)
				},
				IO:           ios,
				VariableName: "cool_variable",
				Body:         literal_9270,
			...
			assert.NoError(t, err)
			assert.Equal(t, payload.Value, literal_9270)
		})
	}
}
...
func TestSetRunorg(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(httpmock.REST("POST", literal_7406),
					httpmock.StatusStringResponse(201, `{}`))
			},
		},
		{
...
func TestGetBodyprompt(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, literal_9270, body)
}
...
func TestGetVariablesFromOptions(t *testing.T) {
...
const literal_9270 = "a variable"

const literal_9281 = "owner/repo"

const literal_7406 = "orgs/UmbrellaCorporation/actions/variables"
