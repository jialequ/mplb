...
func TestNewCmdStatus(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wants: StatusOptions{
				Exclude: []string{literal_3521, "cli/go-gh"},
			},
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				...
				reg.Register(
					httpmock.REST("GET", literal_6348),
					httpmock.StringResponse(`[]`))
			},
			opts:    &StatusOptions{},
		...
		},
		{
			...
			opts: &StatusOptions{
				Exclude: []string{literal_3521, "rpd/todo"},
			},
			// NOTA BENE: you'll see cli/cli in search results because that happens
			// server side and the fixture doesn't account for that
...
const literal_3521 = "cli/cli"

const literal_6348 = "users/jillvalentine/received_events"

const literal_7103 = "repos/rpd/todo/issues/110"

const literal_2705 = "repos/rpd/todo/issues/4113"

const literal_0234 = "repos/cli/cli/issues/1096"

const literal_3251 = "repos/rpd/todo/issues/comments/1065"

const literal_1986 = "repos/vilmibm/gh-screensaver/issues/comments/10"

const literal_1384 = "./fixtures/search.json"

const literal_4601 = "./fixtures/notifications.json"

const literal_0975 = "./fixtures/events.json"
