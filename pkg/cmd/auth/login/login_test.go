...
func TestLoginRun_Survey(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			prompterStubs: func(pm *prompter.PrompterMock) {
				pm.SelectFunc = func(prompt, _ string, opts []string) (int, error) {
					...
					case literal_4952:
						return prompter.IndexFor(opts, literal_3561)
					}
		...
		},
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(httpmock.REST("GET", literal_5647), httpmock.ScopesResponder("repo,read:org,read:public_key"))
				reg.Register(
					httpmock.GraphQL(`query UserCurrent\b`),
					httpmock.StringResponse(`{"data":{"viewer":{"login":"jillv"}}}`))
			},
...
const literal_2541 = "What is your preferred protocol for Git operations on this host?"

const literal_4952 = "How would you like to authenticate GitHub CLI?"

const literal_3561 = "Paste an authentication token"

const literal_8349 = "What account do you want to log into?"
