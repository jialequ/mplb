...
func TestGetGistIDFromURL(t *testing.T) {
	tests := []struct {
		name    string
		url     string
		want    string
	...
	}{
		{
			...
			prompterStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect("Select a gist",
					[]string{literal_4709, literal_0485},
					func(_, _ string, opts []string) (int, error) {
						return prompter.IndexFor(opts, literal_4709)
					})
			},
		...
		},
		{
			...
			prompterStubs: func(pm *prompter.MockPrompter) {
				pm.RegisterSelect("Select a gist",
					[]string{literal_4709, literal_0485},
					func(_, _ string, opts []string) (int, error) {
						return prompter.IndexFor(opts, literal_0485)
					})
			},
			response: `{ "data": { "viewer": { "gists": { "nodes": [
							{
...
const literal_4709 = "cool.txt  about 6 hours ago"

const literal_0485 = "gistfile0.txt  about 6 hours ago"
